...
> ⚠️ **請把這裡的數字當參考，不是定論。** 這是個人專案的實驗紀錄，不是經過審查的 benchmark。作者不保證這套測試程式是「合格、有參考價值」的評測，也不敢說它考慮到了所有因素。已知的限制列在[第 6 節](#6-限制與免責聲明)，引用或據此做決策前請先看過，最好自己重跑驗證。
...
公開這個 repo 的目的是**讓人看到測試方法、能拿程式碼自己驗證**，不是宣稱哪個模型或平台比較好。以下是作者自己知道的不足；不知道的一定更多。

**題組與統計**
- 只有 **8 句**，每句 3 次。樣本太小，6/8 和 4/8 的差距沒有統計意義上的保證，一題之差就是 12.5 個百分點。
- 題目是作者針對自己的記帳情境寫的（繁體中文、台灣用語、自己的帳戶與分類清單），不代表一般 NLU 能力，也不代表其他語言或其他領域。
- 預期答案由作者認定，部分題目有討論空間（例如清單沒有「現金」時應該填 null）。

**公平性**
- 凍結 prompt 是**先在 Android / Gemma 上開發、調到可用之後才凍結**的，iOS 是拿同一份 prompt 直接跑。這對 Gemma 有利，對 Apple 的模型不利。iOS 的低分有一部分反映的是「這份 prompt 不適合它」，不是模型能力的上限。
- 兩個平台的 tokenizer、context 上限、system prompt 機制、結構化輸出的實作都不同，「同一份 prompt」不等於「同樣的條件」。
- iOS 26 因語言閘門只能用偏離規格的 prompt 放法，該輪與其他輪不是同條件。
- iPhone 16 Pro Max 與 18 Pro Max 之間同時差了硬體、SDK、OS build、量測日與模型變體，無法拆開歸因。

**環境與重現性**
- 每個平台只有**一台裝置**，沒有跨機驗證。延遲受溫度、背景負載、OS 剛升級後的索引工作影響，數字只代表當時那台機器的狀況。
- Android 用的是 **Developer Preview** 模型，Apple 的裝置端模型會隨 OS 與機型更換，而且沒有版本號可記。同樣的程式碼過一段時間重跑，結果很可能不同。
- 帳戶名稱已匿名化（同字數替換）。公開版 prompt 與當初實測用的 prompt 有這個差異，理論上可能影響模型輸出，沒有重新實測驗證。

**程式本身**
- 測試程式、評分工具與大部分文件由 AI（Claude Code）協助撰寫，作者審閱過流程與結果，但**沒有經過第三方 code review**，可能有作者沒發現的 bug 或量測偏差。
- 計時只量「送出請求到收到完整回應」的 wall-clock，沒有量耗電、記憶體、發熱，也沒有量連續長時間使用的表現。
- 沒有測語音辨識（STT）這一段，輸入都是打字；實際產品的錯誤率還要疊上 STT 的錯誤。

**免責**：本 repo 依 MIT 授權「現狀」提供，不附任何保證。內容不代表 Google、Apple 或任何廠商的官方立場或效能宣稱。若你發現測試方法有問題，歡迎開 issue 指正，這比任何分數都有價值。
